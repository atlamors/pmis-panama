# [Moved] Frontend Styling Strategy

This page has moved to System Overview → Frontend.

New path: `/system-overview/frontend/frontend-styling-strategy`

## Overview

Our styling approach balances consistency, clarity, and scalability across the host and all MFEs. We lean on Tailwind for expressiveness, CSS variables for design tokens, and Angular’s component CSS for layout. This ensures visual alignment while keeping styles predictable and easy to maintain.
...
- **Tailwind‑first**: Use Tailwind utilities directly in templates for tokens (color, typography, spacing, radius) and interaction states (hover, focus, active).
...
- **Component CSS for structure**: Use `.component.css` only for layout mechanics (flex, grid, positioning), complex animations, or deep overrides of third‑party components.
- **Global styles for system**: `styles.css` also contains resets, theme variables, and cross‑app rules like scrollbars or focus outlines — not component styling.
...
👉 In short: Tailwind handles the look, tokens define the theme, and CSS files handle the structure when utilities fall short.
...
2. Component CSS (`*.component.css`): layout mechanics only (flex/grid, sizing, positioning, overflow, z‑index, keyframes).
...
- Does it change layout mechanics? (display, grid/flex, explicit widths/heights, positioning) → Component CSS
- Is it a design token or state? (color, spacing, radius, border, shadow, hover/focus/active, transitions) → Template utilities
- Is it global theming or reset? (tokens, scrollbar, base font) → Global

### Naming Guidance (layout classes only)

- Use component‑scoped class names for layout hooks: `.card`, `.row`, `.grid`, `.media`, `.meta`.
- Avoid “utility‑sounding” names in CSS (like `.rounded`, `.border`, `.text-*`): those go in the template.
...
- Use data‑driven flags (`@if`, `@for`) to toggle badges/dots/labels rather than duplicating markup.
...
- Example:
  ...
  - HTML: `<section class="p-3 md:p-4 lg:p-6">…</section>`

## Animations

- Complex keyframes → component CSS.
- Simple transitions → inline (`transition`, `duration-200`, `ease-out`).
- Example:
...
- Global “utility” classes in `styles.css` (`.border`, `.rounded`, etc.).
...
👉 In short: Layout in CSS, design style in components, tokens in global `styles.css` — data drives the rest.
